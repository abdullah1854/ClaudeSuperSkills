...
Distinctive, production-grade frontend interfaces with high design quality. Avoids generic AI aesthetics. Use when: building UI components, creating landing pages, implementing responsive designs.
...
## Tags
`frontend`, `design`, `ui`, `ux`, `tailwind`, `components`, `responsive`

## MCP Dependencies
None specified

## Inputs
- `component` (string) (required): Component type: landing, dashboard, form, card, navigation
- `style` (string) (optional): Style: minimal, bold, glassmorphism, brutalist, corporate
- `framework` (string) (optional): Framework: react, nextjs, vue, svelte

## Chain of Thought
1. Understand component requirements. 2. Select distinctive design pattern (avoid generic). 3. Choose color palette with intention. 4. Implement with accessibility in mind. 5. Add micro-interactions. 6. Ensure responsive behavior.
...
gateway_execute_skill({ name: "frontend-design", inputs: { ... } })

// Or via REST API:
// POST /api/code/skills/frontend-design/execute
// Body: { "inputs": { ... } }
```
